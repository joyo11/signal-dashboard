...
**Higher score = worse = fix sooner.** Labels: **70+ = High**, **40–69 = Medium**,
**under 40 = Low**.
...
- **Signal detail drawer.** Click any signal → a panel slides in with its score,
  the score breakdown (55/30/15), its current metrics, and a fault warning if a
...
You can ask it:
- **"What's happening?"** → a summary: the top signal, how many are high-priority,
  how many alerts.
- **"What do I retime first?"** → the worst signal with its numbers and a
  recommendation.
- **"Any alerts?"** → the current anomaly list.
- **"Why SIG-1003?"** (any signal) → that intersection's rank, score, and metrics,
  plus a button to open its detail drawer.
...
It's a keyword-matched helper over the real data (not a free-form AI like ChatGPT),
so every answer is accurate to what's on screen. It also greets first-time visitors
...
**Why sample data?** Real signal data isn't freely downloadable by a program —
agencies make you export it by hand from their website. So the project ships with
...
1. Go to the public UDOT ATSPM site and export the four metrics (split failures,
   approach volume, arrivals-on-red, pedestrian delay) as CSV files — done by hand,
...
## 11. The 30-second pitch (say this in an interview)

> "It's a signal performance dashboard. Traffic signals produce a lot of hourly
> data — volume, arrivals on red, split failures. My tool reads that, scores every
> intersection on how badly it needs retiming, and ranks them so an engineer knows
> what to fix first. It also flags anomalies like detector faults. The analysis is
> real and built to the ATSPM data format, so it runs on real signal data
> unchanged. The goal is to move from reactive — waiting for complaints — to
> proactive, letting the data point you to the problem."
